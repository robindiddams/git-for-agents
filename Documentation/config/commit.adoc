...
`commit.allowAmend`::
	If set to `false`, `git commit --amend` will refuse to run unless
	`--no-verify` is also specified. This provides a client-side safety
	mechanism to prevent AI agents from rewriting commit history.
	Defaults to `true` (amend allowed).
