`push.allowForcePush`::
	If set to `false`, `git push` will refuse to run when `--force`,
	`--force-with-lease`, or `--mirror` options are specified. This
	provides a client-side safety mechanism to prevent accidental
	force pushes. Defaults to `true` (force push allowed).
